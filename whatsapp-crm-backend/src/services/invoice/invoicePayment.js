...
console.log('PhonePe SDK v2 Configuration:', {
  clientId: PHONEPECONFIG.clientId,
...
// // This function is now handled by the SDK, but keep it for reference/other uses
...
// Utility function: Generate Merchant Transaction ID
...
async function initiatePayment(pool, invoiceId) {
  try {
    // Fetch the invoice and customer info from DB
    ...
    // const cleanPhoneNumber = invoice.phonenumber.replace(/\D/g, '').replace(/^91/, ''); // Indian format

    // Use phone_number from invoice or lead depending on which one you use:
    const rawPhoneNumber = invoice.phone_number || ''; // Handles missing value gracefully
    const cleanPhoneNumber = rawPhoneNumber.replace(/\D/g, '').replace(/^91/, '');


    // Compose meta info if needed
    const metaInfo = {
        udf1: invoice.leadname || '',
        udf2: invoice.email || ''
        };

    // const amount = Math.round(parseFloat(invoice.amount) * 100); // PAISA
    ...
    // Log transaction and update DB as needed
    await pool.query(
    ...
    console.error('PhonePe Payment Error:', error.message, error.data || error.response?.data);
...
async function handlePaymentCallback(pool, callbackResponse) {
  try {
    ...
    console.log('Raw Callback Response:', callbackResponse);
    
    let callbackData;
    try {
      const decodedResponse = Buffer.from(callbackResponse, 'base64').toString('utf-8');
      console.log('Decoded Callback:', decodedResponse);
    ...
    console.log('Extracted Callback Info:', {
    ...
    if (state === 'COMPLETED' || state === 'SUCCESS') {
      // Update the invoice with phonepe_reference_id
      await pool.query(
        `UPDATE invoices 
...
//     console.log('PhonePe Complete Response:', JSON.stringify(response, null, 2));

//     const isSuccess = response.state === 'COMPLETED';
//     let paymentMode = '';

//     if (response.payment_details && response.payment_details.length > 0) {
//       const completedPayment = response.payment_details.find(p => p.state === 'COMPLETED');
//       if (completedPayment) {
//         referenceId = completedPayment.transactionId || response.order_id || '';
...
//       }
//     } else {
//       // Fallback to order_id if payment_details not available
//       referenceId = response.order_id || '';
//     }

//     console.log(referenceId)

//     // Method 2: Check if transactionId exists at root level
//     if (!referenceId && response.transactionId) {
//       referenceId = response.transactionId;
//       console.log('Found reference ID at root level:', referenceId);
//     }
    
//     // Method 3: Check order_id (PhonePe's order ID)
//     if (!referenceId && response.order_id) {
//       referenceId = response.order_id;
//       console.log('Using order_id as reference:', referenceId);
//     }


//     // Method 4: Check paymentDetails (camelCase)
...
//         console.log('Found reference ID from paymentDetails (camelCase):', referenceId);
//       }
//     }
...
//       merchantOrderId,
//       invoiceId,
//       hasPaymentDetails: !!response.payment_details,
//       paymentDetailsLength: response.payment_details?.length || 0
//     });

//     if (isSuccess) {

//         console.log('Payment success, calling handleSuccessfulPayment with', {
//         merchantOrderId,
//         referenceId,
//         invoiceId
//         });
//         // Update the invoice with the PhonePe reference ID here
//         await pool.query(
//             `UPDATE invoices SET phonepe_reference_id = $1, updated_at = NOW() WHERE id = $2`,
//             [referenceId, invoiceId]
//         );
//         // Update payment_transactions with payment mode and response data
//       await pool.query(
//         `UPDATE payment_transactions 
//          SET phonepe_transaction_id = $1, 
//              payment_mode = $2,
...
//       );
//     //   await handleSuccessfulPayment(pool, merchantOrderId, referenceId, invoiceId); 
//       await handleSuccessfulPayment(pool, merchantOrderId, referenceId || merchantOrderId, invoiceId);
//       return { success: true, status: 'completed', data: response };
//     } else if (response.state === 'FAILED') {
//       await handleFailedPayment(pool, merchantOrderId, invoiceId);
//       return { success: false, status: 'failed', data: response };
//     }
//     return { success: false, status: response.state, data: response };
//   } catch (error) {
//     console.error('Status check error v2:', error);
//     throw error;
//   }
// }
...
async function checkPaymentStatus(pool, invoiceId, merchantOrderId) {
  try {
    ...
    // Method 1: Extract from payment_details array (snake_case)
    if (response.payment_details && Array.isArray(response.payment_details) && response.payment_details.length > 0) {
      ...
      if (completedPayment) {
        ...
        console.log('Payment Details Found (snake_case):', {
          referenceId,
          paymentMode,
          fullPaymentDetails: completedPayment
        });
    ...
    // Method 2: Check paymentDetails array (camelCase)
    if (!referenceId && response.paymentDetails && Array.isArray(response.paymentDetails)) {
      const completedPayment = response.paymentDetails.find(p => p.state === 'COMPLETED');
      if (completedPayment) {
        referenceId = completedPayment.transactionId || '';
        paymentMode = completedPayment.paymentMode || '';
        console.log('Payment Details Found (camelCase):', { referenceId, paymentMode });
      }
    }

    // Method 3: Check root-level transactionId
    if (!referenceId && response.transactionId) {
      referenceId = response.transactionId;
      console.log('Found reference ID at root level:', referenceId);
    }
    
    // Method 4: Fallback to order_id
    if (!referenceId && response.order_id) {
      referenceId = response.order_id;
      console.log('Using order_id as reference:', referenceId);
    }

    // Method 5: Last resort - use merchantOrderId
    if (!referenceId) {
      referenceId = merchantOrderId;
      console.log('Using merchantOrderId as fallback reference:', referenceId);
    }
    ...
    if (isSuccess) {
      console.log('Payment success, processing with:', {
        merchantOrderId,
        referenceId,
        invoiceId,
        paymentMode
      ...
      try {
        ...
        // Update invoice with PhonePe reference ID
        await client.query(
          `UPDATE invoices 
...
//     await client.query(`
//       UPDATE payment_transactions
//       SET status = 'success', phonepe_transaction_id = $1, updated_at = NOW()
...
//         phonepe_reference_id: phonePeTransactionId
//       })
//     ]);
...
async function handleSuccessfulPayment(client, merchantOrderId, referenceId, invoiceId) {
  try {
    console.log('Processing successful payment:', { merchantOrderId, referenceId, invoiceId });
...
//     // Get the payment transaction details to extract error message
//     const txnResult = await pool.query(
//       `SELECT callback_data FROM payment_transactions WHERE merchant_transaction_id = $1`,
//       [merchantTransactionId]
//     );

//     let errorMessage = 'Payment failed';
//     if (txnResult.rows.length > 0 && txnResult.rows[0].callback_data) {
//       const callbackData = txnResult.rows[0].callback_data;
//       const paymentData = callbackData.data || callbackData.response || {};
//       errorMessage = paymentData.errorCode || paymentData.detailedErrorCode || paymentData.errorMessage || 'Payment failed';
//     }
...
//           updated_at = NOW()
//       WHERE merchant_transaction_id = $2
//     `, [errorMessage, merchantTransactionId]);
...
async function handleFailedPayment(pool, merchantOrderId, invoiceId) {
  try {
    console.log('Processing failed payment:', { merchantOrderId, invoiceId });
    ...
    try {
      ...
      // Update invoice to pending (not failed yet if retries remain)
      await client.query(
        `UPDATE invoices 
         SET status = 'pending',
             updated_at = NOW() 
...
async function schedulePaymentRetry(client, invoiceId, merchantOrderId, retryNumber) {
  const retryDelayMinutes = [30, 120, 360][retryNumber - 1] || 360; // 30min, 2hr, 6hr
...
// Add to invoicePayment.js
async function recordPartialPayment(pool, invoiceId, amount, paymentMethod) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
...
//     const invoice = await client.query(
//       'SELECT * FROM invoices WHERE id = $1 AND status = $2 FOR UPDATE',
//       [invoiceId, 'paid']
//     );
...
//     if (refundAmt > parseFloat(inv.amount)) {
//       throw new Error('Refund amount exceeds invoice amount');
//     }
    
//     // Initiate PhonePe refund
//     const merchantRefundId = `REFUND-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    
//     // Create refund request with PhonePe SDK
//     // Note: PhonePe SDK v2 doesn't have direct refund method in standard checkout
//     // You'll need to use their merchant dashboard or contact support for refunds
//     // For now, we'll mark it internally and process manually
    
//     await client.query(`
//       INSERT INTO refunds (
//         invoice_id, refund_amount, reason, status, 
//         merchant_refund_id, created_at
//       ) VALUES ($1, $2, $3, 'pending', $4, NOW())
//     `, [invoiceId, refundAmt, reason, merchantRefundId]);
...
//       ) VALUES (
//         'refund_request',
//         'Refund Request - Invoice ${inv.invoice_number}',
//         'Refund of ₹${refundAmt} requested for invoice ${inv.invoice_number}. Reason: ${reason}',
//         'high'
//       )
//     `);
...
async function retryFailedPayment(pool, invoiceId, maxRetries = 3) {
  try {
    ...
    // Reset status to allow new payment attempt
    await pool.query(`
      UPDATE invoices 
    ...
    const paymentResult = await initiatePayment(pool, invoiceId);
...
async function processPartialPayment(pool, invoiceId, paidAmount, referenceId) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
...
async function convertCurrency(amount, fromCurrency, toCurrency) {
  // Use a currency conversion API (e.g., exchangerate-api.com)
...
async function createInvoiceWithCurrency(pool, invoiceData, currency = 'INR') {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    if (currency !== 'INR') {
      finalAmount = await convertCurrency(invoiceData.amount, currency, 'INR');
    }
...
function calculateTax(amount, region, itemType = 'service') {
  const taxRates = {
    'karnataka': { sgst: 9, cgst: 9 }, // 18% GST
    'delhi': { sgst: 9, cgst: 9 },
    'maharashtra': { sgst: 9, cgst: 9 },
    'international': { vat: 0 } // No tax for international
  };
